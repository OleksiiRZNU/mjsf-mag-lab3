...
export default class TodoRepository implements TodoRepositoryInterface {
    ...
    create(task: Task): Promise<Task> {
        return new Promise<Task>((resolve, reject) => {
            ...
            const updatedTodos = this.updateTodos(todos);
            resolve(task);
        });
    }
    ...
    fetch(): Promise<Task[]> {
        return new Promise<Task[]>((resolve, reject) => resolve(this.getAllTodos()));
    }
    ...
    save(task: Task): Promise<Task> {
        return new Promise((resolve, reject) => {
            ...
            if (typeof existingRecord !== "undefined"){
                ...
                records[index] = task;
                const updatedRecords = this.updateTodos(records);
    ...
    getAllTodos(): Task[] {
        return localStorage.getItem('todos') ? JSON.parse(localStorage.getItem("todos") as string) : [];
    }
